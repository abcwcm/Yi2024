...
dataset="UVM_GSE139829"
data_dir <- "/athena/abc/scratch/paz2005/projects/2022_07_klebanoff_fei_scRNAseq/datasets/UVM_GSE139829/"
...
#metadata <- fread("UVM_GSE139829_CellMetainfo_table.tsv")
#unique(metadata[,c("Patient", "Source", "Age", "Gender", "Stage")])
metadata <- fread(paste0(data_dir, "UVM_GSE139829_metadata.csv"))
...
cell_annot <- fread(paste0(data_dir, "UVM_GSE139829_CellMetainfo_table.tsv"))
...
setnames(cell_annot, "Patient", "Sample")
...
my_h5_file="UVM_GSE139829_expression.h5"
...
## add metadata
k <- 1
sce.all$Sample = substr(colData(sce.all)$Cell, 1, sapply(gregexpr("@", colData(sce.all)$Cell), "[", k) - 1) %>% gsub(".*_","",.)
